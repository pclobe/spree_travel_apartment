...
describe 'load' do
  ...
  it { expect { Rake::Task['spree_travel_apartment:load'].invoke }.not_to raise_exception }

  context 'inserts correctly' do
    before { Dummy::Application.load_tasks }
    it 'the calculator' do
      expect(Spree::TravelCalculator.where(:name => 'Spree::CalculatorApartment').first.product_type.name).to eq('apartment')
    end

    it 'the option types' do
      Rake::Task['spree_travel_apartment:load'].invoke
